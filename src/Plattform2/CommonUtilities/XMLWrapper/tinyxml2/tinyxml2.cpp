...
namespace tinyxml2
{
	...
	bool XMLUtil::ToWString(const char* str, wchar_t* buffer)
	{
		std::wstringstream ss;
		ss << str;

		for (unsigned int i = 0; i < ss.str().size(); ++i)
		{
			buffer[i] = ss.str().at(i);
		}
...
#pragma warning (pop)
	...
	XMLError XMLAttribute::QueryWStringValue(wchar_t* value) const
	{
		if (XMLUtil::ToWString(Value(), value)) {
			return XML_NO_ERROR;
		}
		return XML_WRONG_ATTRIBUTE_TYPE;
	}
	...
	std::wstring XMLElement::GetWText() const
	{
		if (FirstChild() && FirstChild()->ToText())
		{
			std::wstringstream ss;
			ss << FirstChild()->ToText()->Value();
			return ss.str();
		}
		return 0;
	}
