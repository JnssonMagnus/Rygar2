...
namespace CommonUtilities
{
	...
	public:
		...
		CU::GrowingArray <std::tuple<key, const std::type_info*, void*>>
							GetAllValues();
	...
	template<typename key, short MAXSIZE>
	CU::GrowingArray <std::tuple<key, const std::type_info*, void*>>
	OpaqueDictionary<key, MAXSIZE>::GetAllValues()
	{
		CU::GrowingArray <std::tuple<key, const std::type_info*, void*>> data;
		for (const auto& it : myDataPointers)
		{
			std::tuple<key, const std::type_info*, void*> value = std::make_tuple(it.first, myDataTypes[it.first], it.second);
			data.Add(value);
		}
