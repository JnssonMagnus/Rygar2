...
namespace CommonUtilities
{
	...
	//	Matrix44				operator*(const Vector4<T> &aVector4) const;
		...
		void					CreateProjectionMatrixLH2(T aNearZ, T aFarZ, T aAspectRatio, T aFovAngle);
		void					Translate(const float aX, const float aY, const float aZ);
		void					Translate(const Vector4<float>& aTranslation);
		void					Translate(const Vector3<float>& aTranslation);
		...
		void					SetHeading(const Vector3<float> &aForward, const Vector3<float> &aUp);
		...
		void					SetColumn(const unsigned int aIndex, const Vector4<float> &aVector4);
		void					SetColumn(const unsigned int aIndex, const Vector3<float> &aVector3);
		T&						myX();
		T&						myY();
		T&						myZ();
