...
namespace CommonUtilities
{
	...
	public:
		...
		void				Add(const GrowingArray& aGrowingArray);
	...
	public:
		...
		void				Reserve(int aReservationSize);
		void				Resize(int aNewSize);
		void				Add(T* const aValue);
		void				Add(const GrowingArray& aGrowingArray);
		void				SetSafeMode(const bool aSafeMode);

		template<typename C>
		void				AddSorted(T* const aValue, const C& aComparer);
		void				Insert(T* const aValue, const int aIndex);
		void				Remove(T* const aValue);
		void				Remove(const int aIndex);
		void				RemoveCyclic(T* const aValue);
		void				RemoveCyclicAtIndex(const int aIndex);
	...
	template<class T, int size>
	void GrowingArray<T, size>::Fill(const T& aValue)
	{
	...
	template<class T, int size>
	inline typename GrowingArray<T, size>::iterator GrowingArray<T, size>::begin()
	{
		return &myArray[0];
	}

	template<class T, int size>
	inline typename GrowingArray<T, size>::iterator GrowingArray<T, size>::end()
	{
		return &myArray[myArrayPosition];
	}

	template<class T, int size>
	inline typename GrowingArray<T, size>::const_iterator GrowingArray<T, size>::begin() const
	{
		return &myArray[0];
	}

	template<class T, int size>
	inline typename GrowingArray<T, size>::const_iterator GrowingArray<T, size>::end() const
	{
		return &myArray[myArrayPosition];
	}
	...
	template<class T, int size>
	template<typename C>
	void GrowingArray<T*, size>::AddSorted(T* const aValue, const C& aComparer)
	{
		int i = 0;
		while (i < myArrayPosition && aComparer(myArray[i], aValue) && ++i);
		Insert(aValue, i);
	...
	template<class T, int size>
	void GrowingArray<T*, size>::Insert(T* const aValue, const int aIndex)
	{
		assert(aIndex <= myArrayPosition && aIndex >= 0 && "Index out of bounds when trying to insert value!");
		if (myArrayPosition + 1 >= myArraySize)
		{
	...
	template<class T, int size>
	int GrowingArray<T*, size>::Find(T* const aValue) const
	{
		for (int i = 0; i < myArrayPosition; ++i)
		{
			if (myArray[i] == aValue)
	...
	template<class T, int size>
	void GrowingArray<T*, size>::RemoveCyclic(T* const aValue)
	{
		int index = Find(aValue);
		assert(index <= myArrayPosition && index >= 0 && "RemoveCyclic: Value not found!");
		RemoveCyclicAtIndex(index);
	...
	template<class T, int size>
	T*& GrowingArray<T*, size>::operator[](const int &aIndex) const
	{
		assert(aIndex <= myArrayPosition && aIndex >= 0 && "Out of bounds!");
		return myArray[aIndex];
	}
	...
	template<class T, int size>
	void GrowingArray<T*, size>::Add(T* const aValue)
	{
		if (myArrayPosition >= myArraySize)
		{
			Reserve(myArraySize * 2);
	...
	template<class T, int size>
	void GrowingArray<T, size>::Add(const GrowingArray& aGrowingArray)
	{
		Reserve(myArrayPosition + aGrowingArray.Size());
		for (int i = 0; i < aGrowingArray.Size(); i++)
		{
			Add(aGrowingArray[i]);
		}
	}
	...
	template<class T, int size>
	void GrowingArray<T*, size>::Remove(T* const aValue)
	{
		int index = Find(aValue);
		assert(index != -1 && "Value to remove not found in growingArray!");
		Remove(index);
