...
bool CAkDeviceBase::ClearTaskList(TaskList& in_listTasks)
{
	...
	{
		CAkStmTask * pTask = (*it);
		if ( pTask->IsToBeEntityDestroyed() )
		{
			if ( pTask->CanBeEntityDestroyed() )
			{
...
CAkStmTask * CAkDeviceBase::SchedulerFindNextCachingTask()
{
	...
	{
		if ( (*it)->IsToBeEntityDestroyed() && (*it)->CanBeEntityDestroyed() )
		{
    ...
    )
{
    ...
    while ( it != m_listTasks.End() )
    {
        ...
        if ( (*it)->IsToBeEntityDestroyed() )
        {
			if ( (*it)->CanBeEntityDestroyed() )
			{
				// Clean up.
				CAkStmTask * pTaskToDestroy = (*it);
...
void CAkDeviceBase::CountStreamsInTaskList(TaskList& in_listTasks)
{
    ...
    while ( it != in_listTasks.End() )
    {
		...
		if ( (*it)->ProfileIsToBeEntityDestroyed() 
			&& ( !(*it)->IsProfileNew() 
				|| !(*it)->IsProfileReady() ) )
		{
...
, m_bIsToBeEntityDestroyed( false )
, m_bIsFileOpen( false )
...
AKRESULT CAkStmTask::EnsureFileIsOpen()
{
	...
	if ( m_pDeferredOpenData && !m_bIsToBeEntityDestroyed )
	{
...
{
    AKASSERT( m_bIsToBeEntityDestroyed );
    ...
    )
{
	...
	// Update semaphore.
	if ( IsToBeEntityDestroyed() && CanBeEntityDestroyed() )
	{
		// Requires clean up.
		if ( !m_bRequiresScheduling )
        {
    ...
    else
    {
		if ( AK_StmStatusPending == in_eStatus )
		{
			...
			AKASSERT( !IsToBeEntityDestroyed() || !"Cannot call SetStatus(Pending) after stream was scheduled for termination" );
			SetReadyForIO( true );
			if ( !m_bRequiresScheduling )
			{
				m_bRequiresScheduling = true;
...
void CAkAutoStmBase::UpdateSchedulingStatus()
{
	...
	if ( ( ReadyForIO() && NeedsBuffering( m_uVirtualBufferingSize ) )	// requires a transfer.
		|| ( IsToBeEntityDestroyed() && CanBeEntityDestroyed() ) )	// requires clean up.
    {
        if ( !m_bRequiresScheduling )
        {
			m_bRequiresScheduling = true;
