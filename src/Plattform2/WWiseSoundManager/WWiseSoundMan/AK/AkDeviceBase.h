...
namespace StreamMgr
{
    ...
    public:
        ...
        inline bool IsToBeEntityDestroyed()
        {
            ...
            return m_bIsToBeEntityDestroyed && 
		...
		// Task needs to acknowledge that it can be destroyed. Device specific (virtual). Call only when IsToBeEntityDestroyed().
		...
		virtual bool CanBeEntityDestroyed() = 0;

        // Destroys the object. Must be called only if IsToBeEntityDestroyed() and CanBeEntityDestroyed() returned True.
        inline void InstantDestroy()
		{
			AKASSERT( IsToBeEntityDestroyed() && CanBeEntityDestroyed() );
			// Destroys itself.
        ...
        inline bool ProfileIsToBeEntityDestroyed()        // True when the stream has been scheduled for destruction.
        {
            return m_bIsToBeEntityDestroyed;
        }
		...
		inline void SetToBeEntityDestroyed()
		{
			m_bIsToBeEntityDestroyed = true;
			SetReadyForIO( false );
		}
    ...
    protected:
        ...
        AkUInt8				m_bIsToBeEntityDestroyed  :1; // True when this stream is scheduled to be destroyed.
	...
	protected:
		...
		inline void SetRunning( bool in_bRunning )
		{
			...
			SetReadyForIO( in_bRunning && !m_bHasReachedEof && !m_bIsToBeEntityDestroyed );
		}

		inline void SetReachedEof( bool in_bHasReachedEof )
		{
			m_bHasReachedEof = in_bHasReachedEof;
			SetReadyForIO( m_bIsRunning && !in_bHasReachedEof && !m_bIsToBeEntityDestroyed );
		}
